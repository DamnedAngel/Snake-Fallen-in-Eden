...
1.1. Você pode precisar recompilar o SDCC a partir dos fontes se o binário para seu sistema operacional não estiver disponível. O processo não é complicado, entretanto.
1.2 A compilação do SDCC depende do pacote "Build Essentials" no linux. O OSX já possui todas as dependências.
1.3. Para garantir um processo de compilação mais rápido, configure a compilação com **make config** e remova os módulos que não lhe interessarem. Para o uso na compilação de MSX, apenas os módulos relacionados com o Z80 são necessários.
...
2.1. No Windows, a forma mais fácil de "instalar" é extrair o executável hex2bin.exe do pacote de distribuição e movê-lo para a pasta onde seu sdcc.exe está localizado.
2.2. No Linux e no OSX você provavelmente precisará compilar essas ferramentas (embora algumas pessoas reportaram para mim terem encontrados binários disponíveis para download). Nesse caso, faça download do codigo-fonte e o compile. É rápido e fácil.
...
1.1. Esse arquivo contém, além da árvore de diretórios RECOMENDADA para o desenvolvimento de aplicações MSX, a biblioteca fusion-c e um projeto MSX de exemplo, 100% funcional.
...
4.1. Em algumas instalações, por algum motivo o Visual Studio não consegue encontrar os templates. É um bug do Visual Studio. Se for esse seu caso, seguem algumas coisas que você pode tentar:
4.1.1. Descompacte os templates cada um para um diretório específico. Em princípio, o Visual Studio consegue identificar templates compactados ou armazenados em diretórios. Em alguns casos o VS consegue encontrar apenas os templates em diretórios.
4.1.2. Instale workloads diferentes no VS. Exsitem casos de ambientes nos quais o VS passou a enxergar os templates após instalação dos workloads de C++ e Jogos C++.
